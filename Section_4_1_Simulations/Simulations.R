...
################################################################################
#######.  SIMULATIONS FOR SECTION 4.1 of the paper:
#######. 
...
for (k in 1:dim(vector.p.q)[1])
{
  ...
  {set.seed(set.seed.vector.2[j])
    M.simulated.noisy[[i]][j]<-sample(c(0,1),1,prob=c(vector.p.q[k,1],1-vector.p.q[k,1]))} 
    else {set.seed(set.seed.vector.2[j])
  ...
  {m.plus.simulated[i]<-as.numeric(m.plus.minus(M,vector.p.q[k,1],vector.p.q[k,2],
                                                pi.simulated[[i]])[[2]][1])
  m.minus.simulated[i]<-as.numeric(m.plus.minus(M,vector.p.q[k,1],vector.p.q[k,2],
                                                pi.simulated[[i]])[[2]][2])
  }
  ...
  result.opt.c<-list()
  for (i in 1:length(pi.simulated))
  {result.opt.c[[i]]<-optimal.c(M,vector.p.q[k,1],vector.p.q[k,2],alpha,beta,
                                m.plus.simulated[i],m.minus.simulated[i])
  }
...
vector.p.q<-data.frame(p = v.p, q = rep(v.q,each=length(v.p)))  # 900 combinations 
...
set.seed(k)
set.seed.vector.2<-sample(1:(100*num.positive.bags),size=(M[j]*num.positive.bags),
                          replace=FALSE)
set.seed(k)
set.seed.vector.3<-sample(1:(100*num.negative.bags),size=(M[j]*num.negative.bags),
                          replace=FALSE) 
...
{set.seed(set.seed.vector.2[r*i])
  positive.bag.noisy[[j]][[i]][[r]]<-sample(c(0,1),1,prob=c(vector.p.q[k,1],1-vector.p.q[k,1]))} 
  else {set.seed(set.seed.vector.2[r*i])
...
for (i in 1:num.positive.bags)
{m.plus.simulated.positive.bag[[j]][i]<-as.numeric(m.plus.minus(M[j],vector.p.q[k,1],
                                        vector.p.q[k,2],pi.simulated.positive.bag[[j]][i])[[2]][1])
m.minus.simulated.positive.bag[[j]][i]<-as.numeric(m.plus.minus(M[j],vector.p.q[k,1],
                                        vector.p.q[k,2],pi.simulated.positive.bag[[j]][i])[[2]][2])
}
...
for (i in 1:num.negative.bags)
{m.plus.simulated.negative.bag[[j]][i]<-as.numeric(m.plus.minus(M[j],vector.p.q[k,1],
                              vector.p.q[k,2],pi.simulated.negative.bag[[j]][i])[[2]][1])
m.minus.simulated.negative.bag[[j]][i]<-as.numeric(m.plus.minus(M[j],vector.p.q[k,1],
                            vector.p.q[k,2],pi.simulated.negative.bag[[j]][i])[[2]][2])
}
...
{result.opt.c.positive.bag[[j]][[i]]<-optimal.c(M[j],vector.p.q[k,1],vector.p.q[k,2],
                                              alpha,beta,m.plus.simulated.positive.bag[[j]][i],
                                              m.minus.simulated.positive.bag[[j]][i])
}
...
{result.opt.c.negative.bag[[j]][[i]]<-optimal.c(M[j],vector.p.q[k,1],vector.p.q[k,2],
                                                alpha,beta,m.plus.simulated.negative.bag[[j]][i],
                                                m.minus.simulated.negative.bag[[j]][i])
}
...
heat.map.Delta.success.scenario.2[[j]]<-ggplot(data = Delta.success.join.df[[j]], 

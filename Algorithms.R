...
{ standard.prob.error.I<-pbinom(0, size = M, prob = q, lower.tail = FALSE)   # for c=1
  ...
  if (round(M)!=M | M < 1 | round(m.plus)!=m.plus | m.plus<0 | round(m.minus) != m.minus | m.minus<0 | m.plus+m.minus!= M | 
      p < 0 | p > 0.5 | q < 0 | q > 0.5 | alpha < 0 | alpha > 1| beta < 0 | beta > 1)
  ...
  #stop
    ...
    {print("(h1) fails. No feasible solution for P_c(type I error)<=alpha")
      ...
      # stop
                 ...
                 i<-M
                while (sum.I <=alpha)
                {prob.error.I.c.1<-sum.I
                i<-i-1
                sum.I <- sum.I + choose(M,i)*q^{i}*(1-q)^{M-i}
                }
                c1<-i+1
        ...
        {print("(h2.0) fails. No feasible solution for P_c(type II error)<=beta")
          ...
          prob.error.I.opt.c<-NULL 
          prob.error.II<-NULL
          result = data.frame(cbind(c("c1","Prob Type I error at c1","Prob Type II error at c1"," ", "Standard c=1", 
        ...
        # if (m.minus==0 & p^M<=beta)|(m.minus>0 & q <= 1/(m.minus+1) &  q^{m.minus-1}*(m.minus*(1-q)+q*(1-(1-p)^{m.plus})) <= beta))
        ...
        result = data.frame(cbind(c("c1","Prob Type I error at c1","Prob Type II error at c1"," ", 
                                    "optimal c","Prob Type I error at optimal c","Prob Type II error at optimal c"," ", "Standard c=1",
                                    "Standard prob error type I", "Standard prob error type II"),
                                  c(c1,prob.error.I.c.1,prob.error.II.c.1," ",
...
{ if (round(M)!=M | M < 1 | p < 0 | p > 1 | q < 0 | q > 1 | pi < 0 | pi > 1)
{print("Error in input")
  result<-NULL
  stop} else {
    if (pi < q/(1-p))
